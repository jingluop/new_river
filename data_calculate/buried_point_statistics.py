...
class BuriedPointStatisticsCal:
    ...
    def retention_user_statistics(self, interface_params, filter_condition=''):
        ...
        if filter_condition_sql != '':
            ...
            interface_result.append(
                [{'dateTime': item['dateTime'], 'count': item['count'], 'beforeCount': item['beforeCount']} for item in
                 values])
            for index in range(len(all_day_list)):
                # 处理sql返回
                time = datetime.datetime.strptime(all_day_list[index], '%Y%m%d').strftime('%Y-%m-%d')
    ...
    def user_active_wallet_statistics(self, interface_params, filter_condition=''):
        """绑定钱包活跃用户数统计"""
        if filter_condition in ['platform', 'channel']:
            filter_condition_sql = f' {filter_condition},'
        else:
        ...
        # 拿两个日期之间的所有日期
        all_day_list = self.get_all_day_list(interface_params)
        res = BuriedPointStatistics().user_active_wallet(params=interface_params)
        new_users_sql = db_mysql.select_db(
            BuriedPointSql.active_wallet_user.format(optional_field, optional_field, all_day_list[-1], all_day_list[0],
                                                     filter_condition_sql,
                                                     order_condition))
        sql_result = []
        interface_result = []
    ...
    def bind_wallet_statistics(self, interface_params, filter_condition=''):
        """绑定钱包用户数统计"""
        if filter_condition in ['platform', 'channel']:
            filter_condition_sql = f' {filter_condition},'
        else:
        ...
        # 拿两个日期之间的所有日期
        all_day_list = self.get_all_day_list(interface_params)
        res = BuriedPointStatistics().bind_wallet(params=interface_params)
        new_users_sql = db_mysql.select_db(
            BuriedPointSql.bind_wallet.format(optional_field, all_day_list[-1], all_day_list[0],
                                                     filter_condition_sql,
                                                     order_condition))
        sql_result = []
        interface_result = []
    ...
    def uv_statistics(self, interface_params, filter_condition=''):
        """UV统计"""
        if filter_condition in ['channel']:
            filter_condition_sql = f' {filter_condition},'
        else:
            filter_condition_sql = ''
        order_condition = filter_condition_sql
        ...
        first_try_login_sql = db_mysql.select_db(
            BuriedPointSql.uv.format(optional_field, all_day_list[-1], all_day_list[0],
                                                     filter_condition_sql,
                                                     order_condition))
        sql_result = []
        interface_result = []
...
class TestBuriedPoint:
    ...
    def test_user_active_statistics(self, type, dimension_type):
        if type == 5:
            params = {"type": type, "dimensionType": dimension_type, "startDate": '2022-10-01', "endDate": '2022-10-31'}
        else:
            params = {"type": type, "dimensionType": dimension_type}
        res = BuriedPointStatisticsCal().user_active_statistics(params, dimension_type)
        logger.info('新用户测试数据数据库查询为：{}'.format(res[0]))
        logger.info('新用户测试数据接口返回为：{}'.format(res[1]))
        assert res[0] == res[1]
    ...
    # 1近7日，2近30日，3近一个月，4上一个月
    @pytest.mark.parametrize('dimension_type', ['platform', 'channel', 'all'])
    def test_user_active_wallet_statistics(self, type, dimension_type):
        if type == 5:
            params = {"type": type, "dimensionType": dimension_type, "startDate": '2022-10-01', "endDate": '2022-10-31'}
        else:
            params = {"type": type, "dimensionType": dimension_type}
        res = BuriedPointStatisticsCal().user_active_wallet_statistics(params, dimension_type)
        logger.info('新用户测试数据数据库查询为：{}'.format(res[0]))
        logger.info('新用户测试数据接口返回为：{}'.format(res[1]))
        assert res[0] == res[1]
    ...
    # 1近7日，2近30日，3近一个月，4上一个月
    @pytest.mark.parametrize('dimension_type', ['platform', 'channel', 'all'])
    def test_bind_wallet_statistics(self, type, dimension_type):
        if type == 5:
            params = {"type": type, "dimensionType": dimension_type, "startDate": '2022-10-01', "endDate": '2022-10-31'}
        else:
            params = {"type": type, "dimensionType": dimension_type}
        res = BuriedPointStatisticsCal().bind_wallet_statistics(params, dimension_type)
        logger.info('新用户测试数据数据库查询为：{}'.format(res[0]))
        logger.info('新用户测试数据接口返回为：{}'.format(res[1]))
        assert res[0] == res[1]
    ...
    @pytest.mark.parametrize('type', [1, 2, 3, 4])
    # 1近7日，2近30日，3近一个月，4上一个月
    @pytest.mark.parametrize('dimension_type', ['channel', 'all'])
    def test_uv_statistics(self, type, dimension_type):
        if type == 5:
            params = {"type": type, "dimensionType": dimension_type, "startDate": '2022-10-01', "endDate": '2022-10-31'}
        else:
            params = {"type": type, "dimensionType": dimension_type}
        res = BuriedPointStatisticsCal().uv_statistics(params, dimension_type)
        logger.info('新用户测试数据数据库查询为：{}'.format(res[0]))
        logger.info('新用户测试数据接口返回为：{}'.format(res[1]))
        assert res[0] == res[1]
