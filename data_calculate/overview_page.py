...
class OverViewCal:
    ...
    def calculate_sales_top_10(self, time_type):
        ...
        sales_top_10_sql = db_mysql.select_db(Sql.sales_top_10.format(time_type))
        collection_name_sql = []
    ...
    def calculate_heat_map(self, time_type, total_num=20):
        ...
        rise_num_sql = int(db_mysql.select_db(Sql.heat_map_rise_count.format(time_type))[0]['rise_count'])
        fall_num_sql = int(db_mysql.select_db(Sql.heat_map_fall_count.format(time_type))[0]['fall_count'])
        ...
        rise_list_sql = db_mysql.select_db(Sql.heat_map_rise.format(time_type, int(rise_num)))
